...
const DepthCarousel = ({
  ...
  const layout = useCallback(pos => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const allowBlur = !reducedRef.current && window.innerWidth > 900;
      const blurPx = allowBlur && cfg.blur > 0 ? Math.min(cfg.blur, (back / Math.max(1, cfg.visibleCards)) * cfg.blur) : 0;
  ...
  const scheduleLayout = useCallback(pos => {
    pendingPosRef.current = pos;
    if (frameRef.current) return;
    frameRef.current = requestAnimationFrame(() => {
      frameRef.current = null;
      layout(pendingPosRef.current);
    });
  }, [layout]);
  ...
  const tweenTo = useCallback(
    (target, animate) => {
      ...
      const cfg = cfgRef.current;
      const durationMs = animate && !reducedRef.current ? cfg.duration : 0;
